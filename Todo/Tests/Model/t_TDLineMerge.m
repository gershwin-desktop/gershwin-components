/* t_TDLineMerge.m - ObjectTesting coverage for TDLineMerge. Headless.
 * SPDX-License-Identifier: BSD-2-Clause
 */
#import <Foundation/Foundation.h>
#import "Testing.h"
#include "../../TDLineMerge.m"
...
int main(void)
{
  ...
  /* --- only local changed: local wins, no conflict --- */
  {
    ...
    BOOL conflict = YES;
    NSString *merged = [TDLineMerge mergeBase: base local: local remote: base conflict: &conflict];
  ...
  /* --- only remote changed: remote wins, no conflict --- */
  {
    ...
    BOOL conflict = YES;
    NSString *merged = [TDLineMerge mergeBase: base local: base remote: remote conflict: &conflict];
  ...
  /* --- both sides made the identical edit: no conflict --- */
  {
    ...
    BOOL conflict = YES;
    NSString *merged = [TDLineMerge mergeBase: base local: same remote: same conflict: &conflict];
   ...
   * untouched line elsewhere: neither edit is silently dropped --- */
  {
    ...
    BOOL conflict = NO;
    NSString *merged = [TDLineMerge mergeBase: base local: local remote: remote conflict: &conflict];
