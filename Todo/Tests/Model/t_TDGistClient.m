/* t_TDGistClient.m - ObjectTesting coverage for TDGistClient's request
...
#include "../../TDGistClient.m"

@interface TDFakeTransport : NSObject <TDGistTransport>
{
@public
...
@implementation TDFakeTransport
...
int main(void)
{
  ...
  {
    TDFakeTransport *fake = [[TDFakeTransport alloc] init];
    ...
    TDGistClient *client = [[TDGistClient alloc] initWithGistId: @"abc123"
                                                            token: @"ghp_secret"
                                                        transport: fake];
    NSError *error = nil;
  ...
  /* --- fetch: non-200 is reported as an error, not silently empty --- */
  {
    ...
    TDGistClient *client = [[TDGistClient alloc] initWithGistId: @"missing"
                                                            token: @"t"
                                                        transport: fake];
    NSError *error = nil;
    NSDictionary *result = [client fetchGistWithError: &error];
