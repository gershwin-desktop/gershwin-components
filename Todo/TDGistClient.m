...
NSString * const TDGistClientErrorDomain = @"TDGistClientErrorDomain";

static NSError *
TDError(NSInteger code, NSString *message)
{
  return [NSError errorWithDomain: TDGistClientErrorDomain
                              code: code
                          userInfo: [NSDictionary dictionaryWithObject: message
                                                                 forKey: NSLocalizedDescriptionKey]];
}

@implementation TDGistClient
...
- (NSMutableURLRequest *)requestForMethod: (NSString *)method
{
  ...
  [request setValue: @"Todo/1.0" forHTTPHeaderField: @"User-Agent"];
  ...
  if (data == nil)
    {
      if (error != NULL)
        {
          *error = (connError != nil) ? connError : TDError(-1, @"No response fetching the gist");
        }
      return nil;
    }
      ...
      if (error != NULL)
        {
          ...
          *error = TDError([response statusCode], msg);
        }
      return nil;
    }
  ...
  if (![json isKindOfClass: [NSDictionary class]])
    {
      if (error != NULL && *error == nil)
        {
          *error = TDError(-2, @"Gist response was not a JSON object");
        }
      return nil;
    }
      ...
      if (error != NULL)
        {
          ...
          *error = (connError != nil) ? connError : TDError([response statusCode], msg);
        }
      return NO;
    }
