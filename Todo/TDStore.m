...
#import "TDGistClient.h"
#import "TDURLConnectionTransport.h"
#import "TDPreferences.h"

NSString * const TDStoreErrorDomain = @"TDStoreErrorDomain";

static NSString * const TDListsSubdir = @"Lists";
static NSString * const TDBaseSubdir = @"Base";
static NSString * const TDManifestFilename = @"manifest.plist";

@interface TDStore (Private)
...
@implementation TDStore

+ (instancetype)sharedStore
{
  static TDStore *instance = nil;
  ...
  if (self)
    {
      ...
      _cacheDir = [[libPath stringByAppendingPathComponent: @"Todo"] retain];
...
- (void)removeList: (TDList *)list
{
  ...
  [[NSFileManager defaultManager] removeItemAtPath: [self pathForFilename: [list gistFilename] inSubdir: TDListsSubdir]
                                              error: NULL];
  [self saveManifest];
}
...
{
  NSString *content = [TDMarkdownCodec markdownFromList: list];

  [self writeString: content toFilename: [list gistFilename] inSubdir: TDListsSubdir];
  [self saveManifest];
}
...
- (BOOL)pushWithError: (NSError **)error
{
  ...
  for (TDList *list in _lists)
    {
      [filesToPush setObject: [TDMarkdownCodec markdownFromList: list] forKey: [list gistFilename]];
    }
  ...
  for (filename in filesToPush)
    {
      [self writeString: [filesToPush objectForKey: filename] toFilename: filename inSubdir: TDBaseSubdir];
    }
...
@implementation TDStore (Private)
...
- (void)loadManifest
{
  NSString *path = [_cacheDir stringByAppendingPathComponent: TDManifestFilename];
...
- (void)saveManifest
{
  ...
  [self cacheDirNamed: @"."];
  [manifest writeToFile: [_cacheDir stringByAppendingPathComponent: TDManifestFilename] atomically: YES];
}
...
- (void)loadListsFromCache
{
  ...
  NSString *listsDir = [self cacheDirNamed: TDListsSubdir];
  ...
  for (filename in order)
    {
      NSString *content = [self stringFromFilename: filename inSubdir: TDListsSubdir];
      NSString *name = [filename stringByDeletingPathExtension];

      [_lists addObject: [TDMarkdownCodec listFromMarkdown: content name: name]];
...
- (BOOL)ensureClientWithError: (NSError **)error
{
  NSString *gistId = [TDPreferences gistId];
  NSString *token = [TDPreferences token];

  if ([gistId length] == 0 || [token length] == 0)
    {
      if (error != NULL)
        {
          *error = [NSError errorWithDomain: TDStoreErrorDomain
                                        code: -1
...

  {
    ...
    TDURLConnectionTransport *transport = [[[TDURLConnectionTransport alloc] init] autorelease];

    [_client release];
    _client = [[TDGistClient alloc] initWithGistId: gistId token: token transport: transport];
  }
  return YES;
}
  ...
  for (filename in remoteFiles)
    {
      ...
      NSString *localContent = [self stringFromFilename: filename inSubdir: TDListsSubdir];
      NSString *baseContent = [self stringFromFilename: filename inSubdir: TDBaseSubdir];
      BOOL conflict = NO;
      NSString *merged = [TDLineMerge mergeBase: baseContent
                                           local: localContent
                                          remote: remoteContent
                                        conflict: &conflict];
      NSString *name = [filename stringByDeletingPathExtension];
      TDList *list = [TDMarkdownCodec listFromMarkdown: merged name: name];
      NSUInteger existingIndex = NSNotFound;
      NSUInteger i;

      [self writeString: merged toFilename: filename inSubdir: TDListsSubdir];
      [self writeString: remoteContent toFilename: filename inSubdir: TDBaseSubdir];
      if (conflict)
        {
