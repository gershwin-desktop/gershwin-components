...
/*
 * Where Docket keeps the two pieces of account state it needs: which gist
 ...
 * rest of Docket only ever calls +token/+setToken:, so the storage can
 * change without touching DKStore or DKGistClient).
 */
@interface DKPreferences : NSObject
