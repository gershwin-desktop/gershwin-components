...
static NSUInteger
DKLeadingSpaceCount(NSString *line)
{
  NSUInteger n = 0;
...
static BOOL
DKParseTaskLine(NSString *line, NSUInteger *outIndent, BOOL *outDone, NSString **outBody)
{
  NSUInteger indent = DKLeadingSpaceCount(line);
...
static void
DKExtractMarkers(NSString *body, NSString **outTitle, BOOL *outImportant, NSString **outDueDate)
{
...
static void
DKAppendTaskLine(NSMutableString *out, DKTask *task, NSUInteger indent)
{
...
@implementation DKMarkdownCodec

+ (DKList *)listFromMarkdown: (NSString *)markdown name: (NSString *)name
{
  ...
  DKTask *currentTop = nil;
  DKTask *noteOwner = nil;
  ...
  for (i = 0; i < count; i++)
    {
      ...
      if (DKParseTaskLine(line, &indent, &done, &body) && (indent == 0 || indent == 2))
        {
          NSString *title;
          BOOL important;
          NSString *dueDate;
          ...
          DKExtractMarkers(body, &title, &important, &dueDate);

          task = [DKTask taskWithTitle: title];
          [task setDone: done];
...

      {
        ...
        NSUInteger lineIndent = DKLeadingSpaceCount(line);
...
+ (DKTask *)taskFromInlineText: (NSString *)text
{
  NSString *title;
  BOOL important;
  NSString *dueDate;
  ...
  DKExtractMarkers(text, &title, &important, &dueDate);
  task = [DKTask taskWithTitle: title];
  [task setImportant: important];
  [task setDueDate: dueDate];
  return task;
...
+ (NSString *)markdownFromList: (DKList *)list
{
  NSMutableString *out = [NSMutableString string];

  [out appendFormat: @"# %@\n\n", [list name]];
  for (DKTask *task in [list tasks])
    {
      DKAppendTaskLine(out, task, 0);
      for (DKTask *sub in [task subtasks])
        {
          DKAppendTaskLine(out, sub, 2);
        }
    }
  return out;
}
